...
cinic_directory = './data/cinic10' # Make sure to set the correct path
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.fc_layers = nn.Sequential(
            ...
            nn.Linear(512, 10), # 10 classes in CINIC10
        )
...
train_data_full = ImageFolder(os.path.join(cinic_directory, 'train'), transform=transform)
...
def train(model, optimizer, train_loader, val_loader, test_loader):
    ...
    if optimizer_name == "PSO":
        writer = SummaryWriter('runs/simulate_pso_cinic')
        ...
        writer = SummaryWriter('runs/simulate_adam_cinic')
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            if optimizer_name == "PSO":
                torch.save(best_model_state, "cinic_best_pso_model_weights.pth")
            else:
                torch.save(best_model_state, "cinic_best_adam_model_weights.pth")
...
def train_and_evaluate_fold(model_fold, train_loader_fold, val_loader_fold, test_loader_fold, optimizer_params, fold_number, trial):
    print("TRAIN FOLD  \U0001F3CB")

    optimizer_name = optimizer_params['optimizer_name']

    print(f"TRAIN OPTIMIZER NAME: {optimizer_name}")
    ...
    training_loss_history, validation_loss_history = train(model_fold, optimizer_fold, train_loader_fold, val_loader_fold, test_loader_fold)
    ...
    return val_metric, val_f1_score_macro


def cross_validate(model, train_loader, val_loader, test_loader, optimizer_params, k_folds, trial):
    validation_metrics = []
    f1_scores = []
    
    for fold_number in range(k_folds):
        print(f"\n\nFold: {fold_number}")
        val_metric, f1_score = train_and_evaluate_fold(model, train_loader, val_loader, test_loader, optimizer_params, fold_number, trial)
    ...
    return average_val_metric, average_f1_score
...
def objective(trial, optimizer_name):
    ...
    print(f"Model({optimizer_name}) on:", next(model.parameters()).device)
    ...
    k_folds = 5
    lr = trial.suggest_float('lr', 0.00009, 0.001)  # Learning rate
    ...
    if optimizer_name == "PSO":
        threshold = trial.suggest_float('threshold', 0.18, 0.23, step=0.001)  # Threshold for phase shift
        shift_boundary = trial.suggest_float('shift_boundary', 0.26, 0.28)  # shift_boundary for shift variation
        min_gradient_magnitude = trial.suggest_float('min_gradient_magnitude', 0.16, 0.18)  # Sensitivity to gradient changes
        optimal_weight = trial.suggest_float('optimal_weight', 0.7, 0.8)  # Transition point
        A = trial.suggest_float('A', 0.1, 0.65)  # Amplitude for oscillations
        f = trial.suggest_float('f', 0.75, .99)  # Frequency for oscillations
        phi = trial.suggest_float('phi', 2, math.pi)  # Phase shift
        C = trial.suggest_float('C', 1.2, 1.7)  # Constant for vapor phase
        phase_shift_loss_weight = trial.suggest_float('phase_shift_loss_weight', 0.30, 0.40)  # Weight for phase shift loss
        transition_slope = trial.suggest_float('transition_slope', 0.16, 0.17)  # Transition slope
    ...
    average_val_metric, average_f1_score = cross_validate(model, train_loader, val_loader, test_loader, optimizer_params, k_folds, trial)
...
pso_folder_name = f"cinic_pso_results_{time.strftime('%Y%m%d_%H%M%S')}"
...
adam_folder_name = f"cinic_adam_results_{time.strftime('%Y%m%d_%H%M%S')}"
os.makedirs(adam_folder_name)
adam_study_file_path = os.path.join(adam_folder_name, "study.db")
adam_study_name = "cinic_adam_study"


pso_pruner = MedianPruner(n_startup_trials=4, n_warmup_steps=0, interval_steps=1)
...
study_pso.optimize(lambda trial: objective(trial, 'PSO'), n_trials=6)

study_pso_df = study_pso.trials_dataframe()
study_pso_df.to_csv(os.path.join(pso_folder_name, "trials.csv"), index=False)

adam_pruner = MedianPruner(n_startup_trials=4, n_warmup_steps=0, interval_steps=1)
...
study_adam.optimize(lambda trial: objective(trial, 'Adam'), n_trials=6)
