...
def train(optimizer, train_loader, val_loader, test_loader):
    ...
    epoch_phase_shift_details = []

    writer = SummaryWriter('runs/simulate_cifar')
    ...
    for epoch in range(num_epochs):
        ...
        for batch_x, batch_y in train_loader:
            ...
            def closure():
                ...
                # Extract phase shift loss parameters
                phase_shift_params = optimizer.extract_parameters(optimizer.param_groups[0])
                phase_shift_loss_weight = phase_shift_params[-2]
                weights = [param.data for param in model.parameters()]

                # Calculate the phase shift loss for each weight
                phase_shift_loss_values = [
                    optimizer.phase_shift_loss(weight, phase_shift_params[4], phase_shift_params[5], phase_shift_params[6], phase_shift_params[7], phase_shift_params[8], phase_shift_params[-1])
            ...
            # Call the optimizer step method with the closure to compute gradients
            loss, phase_shift_details = optimizer.step(closure=closure)

            # Append phase_shift_details of the current batch to the accumulating list
            epoch_phase_shift_details.append(phase_shift_details)
            ...
            # Iterate through phase_shift_details to log the values
        ...
        # At the end of the epoch, create the 3D tensor
        # Here, you can aggregate or otherwise process epoch_phase_shift_details
        # to extract the information you want to visualize

        x = torch.Tensor([p for p in epoch_phase_shift_details])  # Replace this with meaningful metrics
        y = torch.Tensor([epoch for _ in range(len(epoch_phase_shift_details))])
        z = torch.Tensor([training_loss_epoch for _ in range(len(epoch_phase_shift_details))])

        mesh = torch.stack((x, y, z), dim=1)
        writer.add_mesh('3D_Fractal_View', mesh, epoch)  # Note that the global_step is now set to epoch

        # Reset the accumulating list for the next epoch
        epoch_phase_shift_details = []
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(best_model_state, "cifar_best_model_weights.pth")
            epochs_without_improvement = 0
        else:
...
def train_and_evaluate_fold(train_loader_fold, val_loader_fold, test_loader_fold, optimizer_params, fold_number, trial):
    ...
    # Create a new optimizer instance for the fold
...
def sphere_equation(params, x, y, z):
    x0, y0, z0, r = params
    return (x - x0)**2 + (y - y0)**2 + (z - z0)**2 - r**2

def fit_sphere(x_values, y_values, z_values):
    # Initial guess (can be tuned)
    initial_guess = (0, 0, 0, 1)
    
    # Perform least squares fitting
    res = least_squares(sphere_equation, initial_guess, args=(x_values, y_values, z_values))
...
def objective(trial):
    ...
    average_val_metric, average_f1_score = cross_validate(train_dataset, val_dataset, test_dataset, optimizer_params, k_folds)
...
folder_name = f"cifar_results_{time.strftime('%Y%m%d_%H%M%S')}"
os.makedirs(folder_name)

# Save the study as a SQLite database file
study_file_path = os.path.join(folder_name, "study.db")
...
study = optuna.create_study(direction='minimize', pruner=pruner, study_name=study_name, storage=f"sqlite:///{study_file_path}")
...
study_df.to_csv(os.path.join(folder_name, "trials.csv"), index=False)
...
pio.write_image(fig, os.path.join(folder_name, "optimization_history.png"))

# Save the parallel coordinate plot
fig = optuna.visualization.plot_parallel_coordinate(study)
pio.write_image(fig, os.path.join(folder_name, "parallel_coordinate.png"))

# Save the contour plot
fig = optuna.visualization.plot_contour(study, params=['lr', 'threshold'])
pio.write_image(fig, os.path.join(folder_name, "contour.png"))

# Save the slice plot
fig = optuna.visualization.plot_slice(study)
pio.write_image(fig, os.path.join(folder_name, "slice.png"))
...
pio.write_image(fig, os.path.join(folder_name, "param_importances.png"))


#  Specific hyperparameters you want to investigate further, like "f" and "transition_slope."
fig = optuna.visualization.plot_contour(study, params=['f', 'transition_slope'])
pio.write_image(fig, os.path.join(folder_name, "custom_contour.png"))

# Individual hyperparameter's effects by "slicing" other hyperparameters' ranges.
fig = optuna.visualization.plot_slice(study, params=['f', 'transition_slope'])
pio.write_image(fig, os.path.join(folder_name, "custom_slice.png"))

# Save the Edwards Plot
fig = optuna.visualization.plot_edf(study)
pio.write_image(fig, os.path.join(folder_name, "edf.png"))

# Save the Intermediate Values Plot
fig = optuna.visualization.plot_intermediate_values(study)
pio.write_image(fig, os.path.join(folder_name, "intermediate_values.png"))
